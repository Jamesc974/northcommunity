...
const fs = require("fs");
const ms = require("ms");
let warns = JSON.parse(fs.readFileSync("./infowarn.json", "utf8"));
...
module.exports.run = async (bot, message, args) => {
  ...
  if(!wUser) return message.reply("❓ Tu as mis son nom ❓");
  ...
  if(!warnchannel) return message.reply("❗ Impossible de trouver le channel 📥");

  warnchannel.send(warnEmbed);

  if(warns[wUser.id].warns == 2){
    let muterole = message.guild.roles.find(`name`, "mute");
    if(!muterole) return message.reply("❗ Désoler le role **mute** existe pas.");
